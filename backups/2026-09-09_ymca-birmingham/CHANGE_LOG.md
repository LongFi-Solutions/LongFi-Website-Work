...
### Entry 17 — 2026-09-09 — Home (30861): top case-study section -> 2-row "Netflix" rail (Jose request). Removed the centered grid; .hcards is now a 2-row horizontal-scroll grid (grid-auto-flow:column, grid-template-rows:auto auto, grid-auto-columns 340px/300px/82vw responsive) = 3 columns x 2 rows = 6 cards visible + peek of the next. Prev/next round arrow buttons that hide at the ends (Netflix-style), native drag/swipe, CSS smooth-scroll. Arrows use scrollLeft assignment (scrollBy/scrollTo were unreliable in the preview pane; scrollLeft works everywhere). All 10 cards. Live-verified: 2 rows, scrollable to 644px, arrows move scroll, prev hidden at start. Repo lf-home.html updated (PREEDIT-rail backup).

### Entry 18 — 2026-09-09 — Home (30861): bottom logo strip clickability. Case-study cards were already <a> links to their pages (customer logos are non-link <div>s), but the strip auto-scrolled with no pause so they were hard to click, and customer cards showed a misleading grab cursor. Added: #lf-home .cases .case-track:hover{animation-play-state:paused} (pause on hover so case-study cards are clickable) and #lf-home .cases .case-card:not(.cslink){cursor:default} (customer logos not clickable-looking). Live-verified: pause rule present, case-study cursor pointer + navigates, customer cursor default. Repo lf-home.html updated.

## ROUND 3 feedback (2026-09-10)
### Entry 19 — YMCA case study (snippet 31640): removed the "$399 carrier revenue generated for the Y" stat (Jose: NEVER publish revenue numbers in ANY case study) and replaced it with "5 / YMCA sites live and connected to date". Removed the redundant white .top header strip (Jose: does not fit under the site nav). Live-verified. Repo lf-cs-ymca-birmingham.html updated. NOTE: testimonial still says "running at two branches" vs the new "5 sites" stat -> flag to Jose to reconcile.
### Entry 20 — GP-09 updated: added hard rule "No revenue numbers in a case study, ever" (2026-09-10, Jose) and "Header strip omitted on website case-study pages" (2026-09-10). File: 00__Compass/05_Global Practices/GP-09_success-cases.md.
### Entry 21 — Home (30861): added "Extreme Networks / Network Partner" card to the "Trusted by leading industry partners" row (now 6 cards). Logo sourced from extremenetworks.com SVG, rasterized to transparent PNG, uploaded to media (id 31666, /wp-content/uploads/extreme-networks-partner-logo.png). Live-verified: 6 cards, logo loads. Repo lf-home.html updated. NOTE: the partner row is DUPLICATED across ~24 snippets (all vertical pages + case studies); only home updated so far -> offer sitewide rollout to Jose.

### Entry 22 — Removed the top .top banner strip from ALL 10 case studies (Jose: remove from all). Live snippets edited + saved (all clean, no Cloudflare): YMCA 31640 (earlier), Wanitta 31561, World Cup 31537, City of Easton 31435, Corporate Offices 31345, Mardi Gras 31343, Bauhaus 31340, JazzFest 31337, Sunset 31335, SILO 31333. Repo lf-cs-*.html all updated (PREEDIT backups in cs-banner-removal/). Matches new GP-09 rule (omit header strip on website case studies).
### Entry 23 — CORRECTION: the "Trusted by leading industry partners" strip is HOME-ONLY (only lf-home.html has it). Other pages share vendor logos only in the separate "Compatible with" wall. So the Extreme Networks card stays on home only; no sitewide rollout needed. Jose confirmed. Testimonial "two branches" left as-is per Jose.
